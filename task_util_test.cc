...
TEST(FillTasksTest, WorksCorrectly) {
  ...
  EXPECT_LT(
      abs(tasks[0]->train_labels_[478].getScalar()-
          expected_task_0.train_labels_[478].getScalar()),
      kDataTolerance);
  EXPECT_LT(
      abs(tasks[1]->train_labels_[478].getScalar() -
          expected_task_1.train_labels_[478].getScalar()),
      kDataTolerance);
  EXPECT_LT(
      abs(tasks[0]->valid_labels_[94].getScalar() -
          expected_task_0.valid_labels_[94].getScalar()),
      kDataTolerance);
  EXPECT_LT(
      abs(tasks[1]->valid_labels_[94].getScalar() -
          expected_task_1.valid_labels_[94].getScalar()),
      kDataTolerance);
...
TEST(FillTaskWithIncrementingIntegersTest, WorksCorrectly) {
  ...
  EXPECT_FLOAT_EQ(dataset.train_labels_[0].getScalar(), 0.0);
  EXPECT_FLOAT_EQ(
      dataset.train_labels_[kNumTrainExamples - 1].getScalar(),
  ...
  EXPECT_FLOAT_EQ(dataset.valid_labels_[0].getScalar(), 0.0);
  EXPECT_FLOAT_EQ(
      dataset.valid_labels_[kNumValidExamples - 1].getScalar(),
...
TEST(FillTaskWithNonlinearDataTest, PermanenceTest) {
  ...
  EXPECT_TRUE(ScalarEq(dataset.train_labels_[0].getScalar(), 1.508635));
  EXPECT_TRUE(ScalarEq(dataset.train_labels_[994].getScalar(), -2.8410525));
  EXPECT_TRUE(ScalarEq(dataset.valid_labels_[0].getScalar(), 0.0));
  EXPECT_TRUE(ScalarEq(dataset.valid_labels_[98].getScalar(), -0.66133333));
}
